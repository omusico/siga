...
public class Report {
	...
	if (filters[3] == null) {
	    return headFilters = "\n" + "Tramo: " + filters[0] + " UC: " + filters[1] + " Ayuntamiento: " + filters[2];
	}else {
	    return headFilters = "\n" + "Tramo: " + filters[0] + " UC: " + filters[1] + " Ayuntamiento: " + filters[2] + " Parroquia/Subtramo: " + filters[3];
	}
    }
	...
	try {
		...
		PdfTemplate tp = cbhead.createTemplate(image.getWidth(), image.getHeight());
		tp.addImage(image);

		cbhead.addTemplate(tp, 520, 775);

		Phrase headPhrase = new Phrase(title + " - " + getDateFormated() + writeFiltersInHeader(filters), bodyBoldStyle);
		...
		for (int row = 0; row < result.getRowCount(); row++) {
			...
			if (result.getValueAt(row, column) != null) {
				...
				value = new Paragraph(valueFormatted,
					cellBoldStyle);
			    }else if (result.getValueAt(row, column).getClass().getName().equalsIgnoreCase("java.sql.Date")) {
				valueFormatted = df.format(result.getValueAt(row, column));
				value = new Paragraph(valueFormatted,
					cellBoldStyle);
			    }else {
				value = new Paragraph(result
