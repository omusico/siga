...
public class CSVCaracteristicasQueries {
	...
	case Areas_Descanso:
	    return "SELECT "
		    ...
		    + " el," + getLocalizationTablesWithSentido()
	...
	case Enlaces:
	    return "SELECT "
		    ...
		    + getLocalizationTables() + getLocalizationWhere()
		    + filters.getWhereClauseByLocationWidgets(true)
		    + " ORDER BY gid";
	...
	case Firme:
	    return "SELECT "
		    + ConsultasFieldNames
			    .getCSVCaracteristicasFieldNames(element)
		    + " FROM " + DBFieldNames.GIA_SCHEMA + "." + element
		    + " el," + " audasa_extgia_dominios.area_mantenimiento am,"
		    ...
		    + " AND el.tramo = tr.id" + " AND el.municipio = mu.id"
		    + filters.getWhereClauseByLocationWidgets(true)
		    + " ORDER BY gid";
	case Valla_Cierre:
	    return "SELECT "
		    + ConsultasFieldNames
			    .getCSVCaracteristicasFieldNames(element)
		    + " FROM " + DBFieldNames.GIA_SCHEMA + "." + element
		    + " el," + getLocalizationTablesWithSentidoAndPF()
	...
	default:
	    return "SELECT "
		    + ConsultasFieldNames
			    .getCSVCaracteristicasFieldNames(element)
		    + " FROM " + DBFieldNames.GIA_SCHEMA + "." + element
		    + " el," + getLocalizationTables() + getLocalizationWhere()
		    + filters.getWhereClauseByLocationWidgets(true)
		    + " ORDER BY gid";
	}
	...
	return " WHERE el.area_mantenimiento = am.id"
		+ " AND el.base_contratista = bc.id" + " AND el.tramo = tr.id"
	...
	return " WHERE el.area_mantenimiento = am.id"
		...
		+ " AND el.tipo_via = tv.id"
		+ " AND el.nombre_via = cast (nv.id as text)"
		+ " AND el.municipio = mu.id" + " AND el.sentido = st.id";
    }
	...
	return " WHERE el.area_mantenimiento = am.id"
		...
		+ " AND el.municipio = mu.id" + " AND el.tipo_via_pf = tvf.id"
