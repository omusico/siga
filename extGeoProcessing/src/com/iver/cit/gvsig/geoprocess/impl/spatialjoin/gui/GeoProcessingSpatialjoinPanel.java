 ...
 * @author jmorell
 */
public class GeoProcessingSpatialjoinPanel extends AbstractGeoprocessPanel
	implements SpatialJoinPanelIF, IWindow{
    ...
    private void changeSelectedItemsNumberJLabel() {
        if (getFirstSelectionCheck().isSelected()) {
        	...
        	ReadableVectorial va =
        			((FLyrVect)layers.
        				getLayer((String)firstLayerCombo.getSelectedItem()))
        				.getSource();
            try {
    ...
    private void changeSelectedItemsNumberJLabel1() {
        if (getSecondSelectionCheck().isSelected()) {
        	...
        	ReadableVectorial va =
        		((FLyrVect)layers.getLayer(selected)).
        		getSource();
            try {
	...
	public JTextField getFileNameResultTextField() {
		if (fileNameResultTextField == null) {
			super.getFileNameResultTextField().
				setBounds(new java.awt.Rectangle(139,18,
											175,18));
		}
	...
	public boolean openSumarizeFunction() {
		//if(fieldDialog == null){
			//fieldDialog = new SpatialJoinNumericFieldSelection(getFirstLayer());
...
}  //  @jve:decl-index=0:visual-constraint="16,26"
