...
public class MapSheetsLayoutTemplate extends Layout implements IMapSheetsIdentified {
	...
	public MapSheetsLayoutTemplate(
			MapSheetGrid gr, ProjectView pv, Layout size_dpi_layout) {
		super();
		setId(System.currentTimeMillis());
	...
	private void updateViewFrameSize() {
		...
		DoubleValue dv3 = null;
		DoubleValue dv4 = null;
		
		try {
			dv3 = (DoubleValue) grid.getTheMemoryDriver().getFieldValue(0, 2);
			dv4 = (DoubleValue) grid.getTheMemoryDriver().getFieldValue(0, 3);
			
			double w_cm = dv3.doubleValue();
			double h_cm = dv4.doubleValue();
	...
	public void init(
		...
		if (right_col_wit_cm < 5) right_col_wit_cm = 5;
		if (right_col_wit_cm > 10) right_col_wit_cm = 10;
		...
		if (has_ov) {
			...
			if (is_test) {
				localFrame = new FFrameOverView();
			} else {
				localFrame = (FFrameOverView)
				FFrameOverViewFactory.createFrameFromName(FFrameOverViewFactory.registerName);
			}
		...
		/*
			// do not add legend
			legendFrame = (FFrameLegend)
			FFrameLegendFactory.createFrameFromName(FFrameLegendFactory.registerName);
			legendFrame.setName("legend");
			legendFrame.setFFrameDependence(viewFrame);
			legendFrame.setBoundBox(new Rectangle2D.Double(
				right_col_pos_cm,
				legen_top_cm,
				right_col_wit_cm,
				legen_hei_cm));
			legendFrame.setLayout(this);
			getLayoutContext().addFFrame(legendFrame, true, false);
		*/
		// ==============================================
		// act fields
	...
	public void update(int i)  throws Exception {
		...
		MapSheetGridGraphic gri = (MapSheetGridGraphic) grid.getGraphic(i);
		updateWithSheet(gri);
	}
