...
public class ClippingProcess extends RasterProcess {
		...
		try {
			...
			if(interpolationMethod != BufferInterpolation.INTERPOLATION_Undefined) {
				...
				if(buffer != null)
					...
					RasterToolsUtil.messageBoxError("El proceso de recorte ha fallado porque el buffer no contenía datos.", this, new NullPointerException());
					return;
				}
			} else {
				try {
	...
	/**
	 * Acciones para poner a NoData los pixels que estén fuera de las
	 * regiones de interés seleccionadas.
	 * @param buffer
	 */
