...
public class DefaultDimension implements IFMapWMSDimension {
	...
	public String valueAt(int pos) throws ArrayIndexOutOfBoundsException {
		if (compiled) {
			...
			if (type == INTERVAL) {
				double minPos = Double.parseDouble(minValue.toString());
				double maxPos = Double.parseDouble(maxValue.toString());
	...
	public void compile() throws IllegalArgumentException {
        ...
        if (type == INTERVAL) {
        	...
        	if (Double.parseDouble(minValue.toString()) > Double.parseDouble(maxValue.toString())) {
        		Object swap = maxValue;
        		maxValue = minValue;
        		minValue = swap;
        	}
