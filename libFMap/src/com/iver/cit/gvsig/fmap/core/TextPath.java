...
public class TextPath {
	 ...
	 * The word is separated using the normal width of the separator glyph. 
	 */
	 ...
	 * @param text, char[]
	 */
	public TextPath(Graphics2D g, FShape path, char[] text, Font font, 
			float characterSpacing, float characterWidth, boolean kerning,
			float leading, int alignment, float wordSpacing, float margin, 
			boolean rightToLeft) {
		this.text = text;
		if (alignment == FConstant.SYMBOL_STYLE_TEXTALIGNMENT_LEFT ||
				alignment == FConstant.SYMBOL_STYLE_TEXTALIGNMENT_RIGHT || 
				alignment == FConstant.SYMBOL_STYLE_TEXTALIGNMENT_CENTERED ||
				alignment == FConstant.SYMBOL_STYLE_TEXTALIGNMENT_JUSTIFY) {
		...
		*/
		...
		if (alignment==FConstant.SYMBOL_STYLE_TEXTALIGNMENT_RIGHT) {
			posList = computeAtRight(gv, pl, text);
		}
		else if (alignment==FConstant.SYMBOL_STYLE_TEXTALIGNMENT_CENTERED) {
	...
	protected FShape softenShape(FShape shape, GlyphVector gv) {
...
//			pos[i][1] = p.getY() - charWidths[i]*Math.sin(pos[i][2]);
	...
	protected static boolean isWordSeparator(char c, char[] wordSeparators) {
		for (char separator:wordSeparators) {
			if (c==separator) {
				return true;
			}
		}
