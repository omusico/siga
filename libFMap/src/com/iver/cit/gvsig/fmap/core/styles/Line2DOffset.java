			...
			while (!pi.isDone()) {
				...
				case PathIterator.SEG_MOVETO:
					if(from == null){
						...
						/* Puede significar un agujero en un polígono o un salto en una linea.
						 * Entonces, consumimos los segmentos que llevamos
						 * y empezamos un nuevo polígono o una nueva linea.
						 */
						try {
	...
	throws ParallelLinesCannotBeResolvedException, NotEnoughSegmentsToClosePathException {
		...
		if (segmentCount > 1) {
			...
			for (int i = 0; i < segmentCount; i++) {
				LineSegment segment = segments.get(0);
				LineEquation eq = offsetLines.get(segment);
				if (i == 0) { //Calculo de la intersección entre el primer segmento y el último
