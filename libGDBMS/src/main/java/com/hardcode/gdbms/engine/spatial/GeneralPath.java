...
public final class GeneralPath implements Shape, Cloneable {
    ...
    /**
     * Creates a new GeneralPath object.
     *
...
loop: 
    ...
    public boolean contains(double x, double y, double w, double h) {
        Crossings c = Crossings.findCrossings(getPathIterator(null), x, y,
                x + w, y + h);
     ...
     * geometry of the outline of this <code>Shape</code>. The iterator for
     ...
     *
     * @exception InternalError if there is not enough memory.
     *
    ...
    public int doHashCode() {
        ...
        for (int i = 0; i < pointCoords.length; i++) {
            value += ((i + 1) * pointCoords[i]); //weight * num
        }
    ...
    public Geometry getJTSGeometry() {
        Coordinate[] coords = new Coordinate[numTypes];
        if ((numTypes == 1) && (pointTypes[0] == SEG_MOVETO)){
            //punto
            coords[0] = new Coordinate(pointCoords[0], pointCoords[1]);
            return gf.createPoint(coords[0]);
        }else if (pointTypes[numTypes-1] == SEG_CLOSE){
            //polígono
            int index = 0;
            for (int i = 0; i < numTypes-1; i++) {
                coords[i] = new Coordinate(pointCoords[index], pointCoords[index + 1]);
                index = index + 2;
            }
            coords[numTypes-1] = coords[0];
