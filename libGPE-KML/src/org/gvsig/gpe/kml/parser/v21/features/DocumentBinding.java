	...
	public Object parse(IXmlStreamReader parser,GPEDefaultKmlParser handler) throws XmlStreamException, IOException {
		...
		while (!endFeature){
			...
			case IXmlStreamReader.START_ELEMENT:
				if (CompareUtils.compareWithOutNamespace(tag,Kml2_1_Tags.OPEN)){
					parser.next();
					String open = parser.getText();						
				}else if (CompareUtils.compareWithOutNamespace(tag,Kml2_1_Tags.NAME)){
					parser.next();
					handler.getContentHandler().addNameToLayer(parser.getText(),layer);
				}if (CompareUtils.compareWithOutNamespace(tag,Kml2_1_Tags.DESCRIPTION)){
					parser.next();
					handler.getContentHandler().addDescriptionToLayer(parser.getText(),layer);
				}else if (CompareUtils.compareWithOutNamespace(tag,Kml2_1_Tags.STYLE)){					
					Object metadata = handler.getProfile().getStyleBinding().parse(parser, handler);
					handler.getContentHandler().addMetadataToLayer(metadata, layer);
//					StyleBinding.parse(parser, handler);
				}else if (CompareUtils.compareWithOutNamespace(tag,Kml2_1_Tags.STYLEMAP)){					
					Object metadata = handler.getProfile().getStyleMapBinding().parse(parser, handler);
					handler.getContentHandler().addMetadataToLayer(metadata, layer);					
			...
			case IXmlStreamReader.END_ELEMENT:
				if (CompareUtils.compareWithOutNamespace(tag,Kml2_1_Tags.DOCUMENT)){
					endFeature = true;
