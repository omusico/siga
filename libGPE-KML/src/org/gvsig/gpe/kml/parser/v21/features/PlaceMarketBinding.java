...
//					String description = parser.getText();
				...
				}else if (CompareUtils.compareWithOutNamespace(tag,Kml2_1_Tags.VISIBILITY)){
					parser.next();
...
//					String styleURL = parser.getText();
					Object element = handler.getProfile().getElementBinding().parse(parser, handler, feature, null);					
					handler.getContentHandler().addElementToFeature(element, feature);
				}else if (CompareUtils.compareWithOutNamespace(tag,Kml2_1_Tags.POINT)){
				...
				}else if (CompareUtils.compareWithOutNamespace(tag,Kml2_1_Tags.LINESTRING)){
				...
				}else if (CompareUtils.compareWithOutNamespace(tag,Kml2_1_Tags.POLYGON)){
				...
				}else if (CompareUtils.compareWithOutNamespace(tag,Kml2_1_Tags.MULTIGEOMETRY)){
				...
				}else if (CompareUtils.compareWithOutNamespace(tag,Kml2_1_Tags.METADATA)){
					MetadataBinding.parse(parser, handler, feature);
				}
				break;
			case IXmlStreamReader.END_ELEMENT:
				if (CompareUtils.compareWithOutNamespace(tag,Kml2_1_Tags.PLACEMARK)){						
					endFeature = true;
