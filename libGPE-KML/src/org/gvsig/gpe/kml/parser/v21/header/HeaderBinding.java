...
import org.gvsig.gpe.kml.parser.KmlWithOutNamespaceComparator;
import org.gvsig.gpe.kml.utils.Kml2_1_Tags;
import org.gvsig.gpe.kml.utils.Kml2_2_Tags;
...
public class HeaderBinding {
	 ...
	 */
	public String parse(IXmlStreamReader parser,GPEDefaultKmlParser handler) throws KmlHeaderParseException{
		String namespace = Kml2_1_Tags.UNKNOWN_VERSION;
		try {			    
		    int eventType = parser.getEventType();
		    while ((eventType != IXmlStreamReader.START_ELEMENT) && (eventType != IXmlStreamReader.END_DOCUMENT)){
		        eventType = parser.next();	
		        System.out.println(parser.getName() + parser.getText());
		    }
		    	    		    
			QName name = parser.getName();
			//For KML without xml header. Set the namespace to 2.2, that doesn't
			//namespace
			if ((name != null) && (Kml2_1_Tags.DOCUMENT.getLocalPart().equals(name.getLocalPart()))){
			    handler.setQNameComparator(new KmlWithOutNamespaceComparator());
			}
			else if (name.getLocalPart() == "") {
				// We try 2.2 (extract document tag)

			    eventType = parser.getEventType();
			    while (eventType != IXmlStreamReader.START_ELEMENT){
			        eventType = parser.next();	
			        System.out.println(parser.getName() + parser.getText());
			        Boolean b = parser.isWhitespace();
			    }
			}else if (!handler.getQNameComparator().equals(name,Kml2_1_Tags.ROOT)){
				// We try to handle 2.0 version also
				System.out.println("Old Header version: " + name);
				handler.setQNameComparator(new KmlWithOutNamespaceComparator());
