...
public class RasterReadOnlyBuffer extends RasterBuffer {
	 ...
	 */
	private void loadPage(int pag) throws InterruptedException {
		try {
			int h = maxY -minY;
			int w = maxX -minX;
			if(page == null) {
				if(bufWidth == 0 && bufHeight == 0) {
					if(pag == nPages - 1)
						bFactory.setAreaOfInterest(minX, (RasterLibrary.blockHeight * pag) + minY, w, heightLastPage);
					else 
						bFactory.setAreaOfInterest(minX, (RasterLibrary.blockHeight * pag) + minY, w, RasterLibrary.blockHeight);
				} else {
					int hPage = Math.round((RasterLibrary.blockHeight * h) / bufHeight);
					if(pag == nPages - 1) {
						int hLastPage = Math.round((heightLastPage * bufHeight) / h);
						bFactory.setAreaOfInterest(minX, (hPage * pag) + minY, w, hLastPage, bufWidth, heightLastPage);
					} else 
						bFactory.setAreaOfInterest(minX, (hPage * pag) + minY, w, hPage, bufWidth, RasterLibrary.blockHeight);
				}
				...
				if(bufWidth == 0 && bufHeight == 0) {
					if((pag + 1) == nPages - 1)
						bFactory.setAreaOfInterest(minX, (RasterLibrary.blockHeight * (pag + 1)) + minY, w, heightLastPage);
					else 
						bFactory.setAreaOfInterest(minX, (RasterLibrary.blockHeight * (pag + 1)) + minY, w, RasterLibrary.blockHeight);
				} else {
					int hPage = Math.round((RasterLibrary.blockHeight * h) / bufHeight);
					if(pag == nPages - 1) {
						int hLastPage = Math.round((heightLastPage * bufHeight) / h);
						bFactory.setAreaOfInterest(minX, (hPage * (pag + 1)) + minY, w, hLastPage, bufWidth, heightLastPage);
					} else 
						bFactory.setAreaOfInterest(minX, (hPage * (pag + 1)) + minY, w, hPage, bufWidth, RasterLibrary.blockHeight);
				}
