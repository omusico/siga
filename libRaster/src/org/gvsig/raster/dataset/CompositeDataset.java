...
public class CompositeDataset implements IRasterDataSource {
	 ...
	 */
	public IBuffer generateBuffer(IBuffer[][] bufList, int drawableBands) throws InterruptedException {
		int n = mosaic.length;
		int m = mosaic[0].length;
		int nCols = 0, nRows = 0;
	 ...
	 * @throws InterruptedException 
	 */
	private void copyTile(IBuffer origin, IBuffer dest, int r, int c) throws InterruptedException {
	...
	public IBuffer getWindowRaster(double ulx, double uly, double lrx, double lry) 
		...
		try {
			...
			IBuffer endBuffer = generateBuffer(bufferList, mosaic[0][0].getBands().getDrawableBandsCount());
			for (int row = 0; row < n; row++) 
				for (int col = 0; col < m; col++) 
	...
	public IBuffer getWindowRaster(double ulx, double uly, double w, double h, boolean adjustToExtent) 
		throws InvalidSetViewException, RasterDriverException, InterruptedException {
		try {
			MultiRasterDataset[][] datasetList = getDatasetListInArea(ulx, uly, ulx + w, uly - h);
			int n = mosaic.length;
			int m = mosaic[0].length;
			IBuffer[][] bufferList = new IBuffer[n][m];
			for (int row = 0; row < n; row++) 
				for (int col = 0; col < m; col++) 
					if(datasetList[row][col] != null)
						bufferList[row][col] = datasetList[row][col].getWindowRaster(ulx, uly, ulx + w, uly - h);
			IBuffer endBuffer = generateBuffer(bufferList, mosaic[0][0].getBands().getDrawableBandsCount());
			for (int row = 0; row < n; row++) 
				for (int col = 0; col < m; col++) 
					if(bufferList[row][col] != null)
	...
	public boolean needEnhanced() {
		...
		for (int row = 0; row < n; row++) {
			for (int col = 0; col < m; col++) {
				if(mosaic[row][col] != null) {
					if(mosaic[row][col].needEnhanced())
						return true;
				}
			}
		}
