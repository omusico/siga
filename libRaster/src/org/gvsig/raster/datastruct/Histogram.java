			...
			if (bands[iBand]) {
				...
				for (i = beginPos; aux <= stopPos; i++) {
					...
					try {
						...
						Logger.getLogger(getClass().getName()).debug("Acceso a un valor de histogram (Banda: " + iBand + " Posición: " + i + ") fuera de límites. (NBandas: " + histogram.length + " Valores:" + histogram[iBand].length + ")", e);
					}
				}
