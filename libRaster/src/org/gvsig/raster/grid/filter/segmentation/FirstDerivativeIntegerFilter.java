 ...
 */
public class FirstDerivativeIntegerFilter extends FirstDerivativeFilter {
	/*
	 * (non-Javadoc)
			...
			if ((col - semiLado >= 0) && (line - semiLado >= 0) && (col + semiLado < width) && (line + semiLado < height)) {
				...
				for (int j = -semiLado; j <= semiLado; j++)
					for (int i = -semiLado; i <= semiLado; i++)
						ventana[i + semiLado][j + semiLado] = raster.getElemInt(line + j, col + i, band) & 0xffffffff;
				...
				rasterResult.setElem(line, col, band, (byte) raster.getElemInt(line, col, band));
			}
		}
	}
